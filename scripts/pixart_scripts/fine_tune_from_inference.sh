...
# "$PYTHON_BIN" scripts/pixart_scripts/backfill_pixart_inputs.py --base_dir "$BASE_PIXART" --dataset Gazoche/gundam-captioned --split train --image_size 512

# echo "[2/6] Generating metadata.jsonl in $INPUT_DIR ..."
# "$PYTHON_BIN" scripts/pixart_scripts/make_imagefolder_metadata.py --input_dir "$INPUT_DIR"

# echo "[3/6] Cleaning conflicting packages (xformers/flash-attn) ..."
# bash scripts/pixart_scripts/clean_conflicting_pkgs.sh

# echo "[4/6] Installing training dependencies ..."
# bash scripts/pixart_scripts/install_training_deps.sh
...
# "$PYTHON_BIN" scripts/pixart_scripts/build_pixart_combined_base.py --out_dir "$COMBINED_BASE"
...
#   VALIDATION_PROMPT="$VALIDATION_PROMPT" bash scripts/pixart_scripts/train_pixart_lora_from_inputs.sh

echo "[7/7] Validating LoRA and generating new inferences + FID ..."
NEW_OUT_DIR=${NEW_OUT_DIR:-local_repo/PixArt_ft/output_new}
"$PYTHON_BIN" scripts/pixart_scripts/validate_finetune.py --base_combined "$COMBINED_BASE" --lora_dir "$OUTPUT_DIR" --out_dir "${NEW_OUT_DIR}/validation"
"$PYTHON_BIN" scripts/pixart_scripts/infer_finetuned_on_new_gundam.py --base_combined "$COMBINED_BASE" --lora_dir "$OUTPUT_DIR" --out_dir "$NEW_OUT_DIR" --num_samples 50 --skip_first_n 1028
"$PYTHON_BIN" scripts/pixart_scripts/compute_fid_for_folder.py --generated_dir "$NEW_OUT_DIR" --start 1028 --count 50
